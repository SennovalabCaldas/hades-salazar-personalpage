...
const createAcademicExperience = async (req, res) => {
  try {
    ...
    const result = await pool
      .promise()
      .query(
        "INSERT INTO academicExperience (academic_name, academic_description, academic_image, year_of_experience) VALUES (?, ?, ?, ?)",
        [
          academic_name,
          academic_description,
          academic_image,
...
const getAcademicExperiences = async (req, res) => {
  try {
    const result = await pool
      .promise()
      .query("SELECT * FROM academicExperience");
    const rows = result[0];
...
const getAcademicExperienceById = async (req, res) => {
  ...
  try {
    const result = await pool
      .promise()
      .query("SELECT * FROM academicExperience WHERE id = ?", [id]);
...
const updateAcademicExperience = async (req, res) => {
  const { id } = req.params;
  const { academic_name, academic_description, year_of_experience, active } =
    req.body;

  try {
    const result = await pool
      .promise()
      .query("SELECT academic_image FROM academicExperience WHERE id = ?", [
        id,
      ]);
    ...
    await pool
      .promise()
      .query(
        "UPDATE academicExperience SET academic_name = ?, academic_description = ?, year_of_experience = ?, academic_image = ?, active = ? WHERE id = ?",
        [
          academic_name,
          academic_description,
          year_of_experience,
...
const deleteAcademicExperience = async (req, res) => {
  try {
    ...
    await pool
      .promise()
      .query("DELETE FROM academicExperience WHERE id = ?", [id]);
    await fs.unlink(imagePath);
...
const updateStateAcademicExperience = async (req, res) => {
  ...
  try {
    await pool
      .promise()
      .query("UPDATE academicExperience SET active = ? WHERE id = ?", [active, id]);
